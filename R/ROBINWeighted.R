...
randomWeight <- function(graph, verbose=FALSE)
{
  ...
  graphRandom <- as.matrix(perturbR::rewireR(sym.matrix=adj, nperturb=numberPerturbAll, dist = "NegBinom"))
  #rewiring for z all the edges
  graphRandom <- graph_from_adjacency_matrix(graphRandom,weighted = TRUE, mode="undirected")
  return(graphRandom)
}

plotRobin2 <- function(graph, model1, model2,
...
{  
  ...
  plotModel <- ggplot2::ggplot(dataFrame, aes(x = percPert, y = as.numeric(as.character(mvi)),
  ...
  plot <-  plotModel+ggplot2::scale_colour_manual(values = cols,breaks = c("model1", "model2"), labels=c(legend[1], legend[2]))
