...
method="louvain"
#method="fastgreedy"
if(method=="louvain") dir_input="/media/defeis/data/defeis_work/PUBLICATIONS/2014/ERCIM/LuisaItaliaAnna-LIApaper/results_2016/results_louvain_null_model_rewire/SIMULATED_DATA/"
if(method=="fastgreedy") dir_input="/media/defeis/data/defeis_work/PUBLICATIONS/2014/ERCIM/LuisaItaliaAnna-LIApaper/results_2016/results_fastgreedy_null_model_rewire/SIMULATED_DATA/"

ccc=data.frame()
ccc1=data.frame()
ccc2=data.frame()

modc<-as.character(seq(0,0.8,by=0.025))
mod<-gsub('[.]', '', modc)
for(i in c(1:length(mod))){

  name_random=paste(dir_input,"sim",mod[i],"_",method,"_VI_random_bio.txt",sep="")
  name_case=paste(dir_input,"sim",mod[i],"_",method,"_VI_case_bio.txt",sep="")
  
  random_bio <- read.delim(name_random, header=FALSE)
  case_bio <- read.delim(name_case, header=FALSE)
  
  file_out_plot=paste(dir_input,"fda_test_sim",mod[i],"_",method,".eps",sep="")
  file_out_plot1=paste(dir_input,"fda_test_sim",mod[i],"_",method,"_VI.eps",sep="")
  file_out_plot2=paste(dir_input,"fda_test_sim",mod[i],"_",method,"_VI_functional.eps",sep="")
  file_out_plot3=paste(dir_input,"fda_test_sim",mod[i],"_",method,"_pvalues.eps",sep="")
  
  file_out_test=paste(dir_input,"FAD_test_sim",mod[i],"_",method,".txt",sep="")
  ...
  alpha=0.05
#   
#    cc=fpca.sc(rbind(case_bio,random_bio))
#    cc1<-((cc$scores/sqrt(21)))[1:10,]
#    cc2<-((cc$scores/sqrt(21)))[11:20,]
#    pippo1=testAD(cc1,cc2)$pval
...
#  pdf(file_out_plot)
  ...
  colors <- numeric(n)
  colors[which(object$labels==1)] <- "red"#col[1]
  colors[which(object$labels==2)] <-  "blue"#col[2]
  difference1 <- which(pvalues_ad<alpha1)
...
#  pdf(file_out_plot1)
  ...
  legend(0.6,0.3,c(as.expression(~VIc[random]),as.expression(~VIc)),lty=c(1,1),lwd=c(2,2),col=c("blue","red"))
...
#    "gray80"
  ...
  ccc=rbind(ccc,ITP.result$pval)
  ccc1=rbind(ccc1,ITP.result$corrected.pval)
  ccc2=rbind(ccc2,pvalues_ad)
