...
detection algorithm and compares the robustness of two community detection 
algorithms.
...
plotCommu(graph=graph, members=members)
```

# Procedure to validate the robustness
It creates the ROBIN procedure to validate the robustness of your network.
In this example we used the "vi" distance as a stability measure and the louvain 
algorithm as a community detection algorithm, but users can choose different 
measures ("nmi","split.join", "adjusted.rand") and algorithms (walktrap", 
"edgeBetweenness", "fastGreedy", "spinglass", "leadingEigen", "labelProp", 
"infomap", "optimal", "other") implemented in the package.
To make things easier we save the output list in the *proc* variable because 
to use it later.

```{r}
proc <- robin(graph=graph, graphRandom=graphRandom, measure="vi", 
...
The *model* and *modelR* arguments are, respectively, the *MeanReal* and the 
*MeanRandom* contained inside the **robin** output list.

```{r}
plotRobin(graph=graph, model=proc$MeanReal, modelR=proc$MeanRandom, measure="vi")
```
...
The *model1* and *model2* arguments are, respectively, the *MeanReal* and the 
*MeanRandom* contained inside the **robin** output list.
...
plotCommu(graph=graph, members=membersFast)
plotCommu(graph=graph, members=membersLouv)
```

Secondly, we run the **robinComp** function to compare the two algorithms, 
which, as before, we store the output in the *comp* variable to use it later.

```{r}
comp <- robinComp(graph=graph, graphRandom=graphRandom, method1="fastGreedy",
                method2="louvain", measure="vi", type="independent")
```

Thirdly, we plot the curves of each null model and real data for both
methods and the two curves of the two methods in comparison.
The *model1*, *model2*, *modelR1* and *modelR2* arguments are, respectively, 
the *Mean1*, *Mean2*, *MeanRandom1* and *MeanRandom2* contained inside 
the **robinComp** output list.

```{r}
plotRobinComp(graph=graph, model1=comp$Mean1, model2=comp$Mean2,
                 modelR1=comp$MeanRandom1, modelR2=comp$MeanRandom2,
                 measure="vi",
