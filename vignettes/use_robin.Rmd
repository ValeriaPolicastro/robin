...
the *viMean1*, *viMean2*, *viMeanRandom1* and *viMeanRandom2* contained inside 
...
*viMean1* and the *model2* argument must be the *viMean2* both contained inside 
the **comparison** output list.

```{r}
robinFDATest(graph=graph, model1=comp$Mean1, model2=comp$Mean2, measure="vi")
robinGPTest(ratio=comp$ratios1vs2)
robinAUCTest(graph=graph, model1=comp$Mean1, model2=comp$Mean2, measure="vi")
```
